...
public class Atomiic extends BasicMQ {
    public Atomiic() {
        ensureConnection();
    }
                ...
                try {
                    ...
                    while( true ) {
                        try {
                            ...
                            UUID uuid = UUID.randomUUID(); // later we use this to check that it coomes from the same place.
                            Command c = atomiic.reply(uuid);
                            Log.info("Command Received:" + c.getSymbol() + ":" + c.getResult());
                            atomiic.send(c);
                            Log.info("Command Sent:" + c.getSymbol() + ":" + c.getResult());
                            Utility.lag();
                    ...
                    try {
                        String wh = WebhookDirectory.lookup("discord-atomiic-messages");
                        Messenger.notify(wh, "Atomiic Queue Down", "Queue has gone down for Atomiic do not send any transactions");
    ...
    public Command reply(UUID uuid) {
        ensureConnection();
        Channel channel = null;
        try {
            channel = connection.createChannel();
            ...
            while (flag) {
                try {
                    ...
                    if( c.getName() != null && c.getName().equals("getbalance")){
                        DecimalFormat df = new DecimalFormat("0.00000000");
                        String balance = df.format(coin.getBalance());
                        Log.info("Sending back balance:" + balance);
                        c.setResult(balance);
                    } else if( c.getName() != null && c.getName().equals("getbalancebyaddress")){
                        DecimalFormat df = new DecimalFormat("0.00000000");
                        String balance = df.format(coin.getBalanceByAddress(c.getParameter()));
                        Log.info("Sending back balance:" + balance);
                        c.setResult(balance);
                    } else {
                        String address = coin.generateNewAddress();
                        Log.info("Setting generate address:" + address);
                        c.setResult(address);
                    }
                   return c;
